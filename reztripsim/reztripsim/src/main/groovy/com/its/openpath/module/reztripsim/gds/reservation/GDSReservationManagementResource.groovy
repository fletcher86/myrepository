...
class GDSReservationManagementResource
{
  ...
  def Response manageReservation( @Context HttpServletRequest servletRequest )
  {
    ...
    try
    {
      if ( !responseJSON )
      {
        ...
        // NEW, CANCEL, IGNORE, and END Res will only have 1 RoomReservationRequest in the List, MODIFY Res will have 2
        if ( !responseJSON )
        {
    ...
    try
    {
      ...
      switch ( reservationManagementType )
      {
        ...
        case ReservationManagementType.MODIFY_RESERVATION:
          populateReservationModificationResponse( reservationMgtResponse )
          break
  ...
  /**
   * Create and populate a Response to a Reservation Modification Request received.
   * <p />
   * @param reservationMgtResponse - response to populate with the reservation confirmation and other data
   */
  def populateReservationModificationResponse( RoomReservationManagementResponse reservationMgtResponse )
  {
    ...
    resConfirmation.confirmationNumber = '1234-MODIFY-RES-CONF'

    if ( sLogger.isDebugEnabled() )
    {
      sLogger.debug "*** REZTRIPSIM - Built a Reservation Modification Response for: [${InvocationContext.instance.getSessionDataItem( "TXN_REF" )}]"
    }
  }
