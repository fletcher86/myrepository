...
/**
 * <code>RateManagementRequestHandler</code>
 * <p/>
 * Implements the Rate Management Service Provider functionality by interfacing with the RezTrip CRS.
 ...
 * is published on a temporary Topic on the OPS Message bus so that the module that generated the Rate Management
...
@Service("RateManagementRequestHandler")
@ManagedResource('OPENPATH:name=/module/opsari/reztrip/handler/RateManagementRequestHandler')
class RateManagementRequestHandler
extends AbstractServiceRequestHandler
implements IMessageBusSubscriber
{
  private static final Logger sLogger = LoggerFactory.getLogger( RateManagementRequestHandler.class.name )


  /**
   * Constructor
   */
  RateManagementRequestHandler( )
  {
  ...
  void init( )
  {
    mOpsMessageBus.consumeFromQueue( "RATE_REQ", this, false )
  }
  ...
  void onMessage( final String s, final OpsMessage opsMessage )
  {
    ...
    responseJSON = super.postRateMgtRequestToRezTripCRS( requestJSON, requestObj, handleResponseFromRezTrip ) { String endpointURL, int httpStatusCode, Throwable exception ->
